...
using System.Timers;
/// <summary>
/// Read memory :                     ./MicroserviceSLMPControllerClient.exe 169.254.210.13 2000 -r
/// Write 1 words vào addrress 5000:  ./MicroserviceSLMPControllerClient.exe 169.254.210.13 2000 -ws 5000 100
/// Write n words từ address 5000 :   ./MicroserviceSLMPControllerClient.exe 169.254.210.13 2000 -wm 5000 100,200,300,4000
/// </summary>
namespace MicroserviceSLMPControllerClient
{
  class Program
  {
    ...
    static int _port = 2000;
	  static string start_address_1 = "0";
    static string start_address_2 = "0";
    ...
    static string cmd = "-r";
    static int write_single_address = (-1);
    static int write_single_value = (-1);
    static List<int> write_multi_values = new List<int>();
    ...
    {      
      ...
      for (int i = 0; i < args.Length; i++)
      {
        ...
        _ip_address = (i== 0)?args[i]: _ip_address;
        if (i == 1)
        {
          try
          {
        ...
        //
        cmd = (i == 2) ? args[i].Trim() : cmd;

        if ((i == 3) && ((cmd == "-ws") || (cmd == "-wm")))
        {
          try
          {
            write_single_address = int.Parse(args[i]);
          }
          catch
          {
          }          
        ...
        if (i == 4)
        {
          if (cmd == "-ws")
          {
            try
            {
              write_single_value = int.Parse(args[i]);
            }
            catch
            {
            }
          ...
          else if (cmd == "-wm")
          {
            ...
            try
            {
              ...
              {
                write_multi_values.Add(int.Parse(val));
              }
              //write_single_value = int.Parse(args[i]);
            }
            catch
            {
              write_multi_values = new List<int>();
            }
          }
        }
      }
      ...
      if (_clientSocket != null)
      {
        if (_clientSocket.Connected == true)
        {
          if (cmd == "-r")
          {
            ...
            {
              ReadsDeviceMemory_Binary_codes(PROTOCOL_UNIT._x1_WORD, "D" + start_address_1, 500);
              _eBlock = eBlock.Block_2;
            }
            else
            {
              ReadsDeviceMemory_Binary_codes(PROTOCOL_UNIT._x1_WORD, "D" + start_address_2, 500);
              _eBlock = eBlock.Block_1;
            }
          }
          else if ((cmd == "-ws") && (write_single_address >= 0) && (write_single_value >= 0))
          {
            //Write single
            Console.WriteLine($"Write single-value: D{write_single_address}: {write_single_value}");
            WriteDeviceMemory_Binary_codes($"D{write_single_address}", new int[1] { write_single_value }, 1);
          }
          else if ((cmd == "-wm") && (write_single_address >= 0) && (write_multi_values.Count > 0))
          {
            //Write single
            Console.WriteLine($"Write multiles-values: D{write_single_address}: {write_multi_values[0]}");
            WriteDeviceMemory_Binary_codes($"D{write_single_address}", write_multi_values.ToArray(), write_multi_values.Count);
          }
        }
        else
        {
        ...
        else
        {
          ...
          if (Response_data_length > 0)
          {
            ...
            for (int i = 0; i < Response_data_length; i += 2)
            {
              ...
              if (memory_address - _start_address < data.Length)
              {
                data[memory_address - _start_address] = value_from_plc;
              }
